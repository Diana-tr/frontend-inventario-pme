...
const FacturaCompraController = (() => {
  const TABLE_BODY_ID = "tablaFacturasBody";
  const TABLE_ID = "tbl_FacturasCompras";
  const DETAIL_MODAL_ID = "modalVisualizarFactura";
  ...
  function getStatusBadge(status) {
    const badges = {
      DRAFT: '<span class="badge badge-warning px-3 py-2" style="border-radius:20px;font-size:0.75rem">Borrador</span>',
      ISSUED: '<span class="badge badge-success px-3 py-2" style="border-radius:20px;font-size:0.75rem">Emitida</span>',
      CANCELLED: '<span class="badge badge-danger px-3 py-2" style="border-radius:20px;font-size:0.75rem">Anulada</span>'
    };
    return badges[status] || `<span class="badge badge-secondary px-3 py-2" style="border-radius:20px;font-size:0.75rem">${status}</span>`;
  }
      ...
      .on("click", ".btn-view-invoice", async function (e) {
        ...
        try {
          ...
          if (response && response.success && response.data) {
            ...
            $("#factura_modal_date").text(factura.issue_date ? formatDate(factura.issue_date) : "N/A");
            ...
            if (typeof DocumentTemplateRenderer !== 'undefined') {
              ...
              const normalizedDocumentData = {
                ...
                customer_name: factura.supplier_name || "Proveedor",
              ...
              if (factura.template) {
                if (factura.template.body_content) {
                   ...
                   // Cargar ítems legacy style
                   renderInvoiceItems(factura.items, "#factura_modal_items");
                }
              }
            } else {
               console.warn("DocumentTemplateRenderer not found.");
            }
          ...
          console.error("[FACTURAS] Error al obtener detalles:", error);
    ...
    dataTableInstance = $(`#${TABLE_ID}`).DataTable({
      ...
      ajax: async function (data, callback) {
        try {
          ...
          const response = await CompraService.listarFacturasPaginadas({
            page,
            page_size: pageSize,
            search,
            ordering,
          ...
          if (response?.success && response.data) {
            ...
            const rows = facturas.map((factura) => {
              ...
              return [
                ...
                factura.purchase ? `Compra #${factura.purchase}` : `<span class="text-muted">N/A</span>`,
          ...
          console.error("[FACTURAS] Error server-side:", error);
          callback({ draw: data.draw, recordsTotal: 0, recordsFiltered: 0, data: [] });
        }
      },
...
export default FacturaCompraController;
